...
class WebEntityModelGenerator extends GeneratorExecutor implements IGeneratorExecutor {
	...
	def CharSequence generateEntityDefaultAutoComplete(Entity entity, boolean isAutoComplete) {
		val autoCompleteSlots = entity.slots.filter[!mapped].filter[it.isAutoCompleteResult || it.isAutoCompleteData || (entity.hasEntityVersion && it.name.toLowerCase == 'version')]
		'''
	...
	def CharSequence generateListFilterField(Slot slot) {
		...
		«ENDIF»
		«IF isMany»
		«fieldName»: «IF isEnum»«slot.toWebType»«ELSE»«slot.toAutoCompleteClassName»«ENDIF»[];
