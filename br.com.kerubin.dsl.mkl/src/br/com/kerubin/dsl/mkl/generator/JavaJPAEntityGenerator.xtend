...
class JavaJPAEntityGenerator extends GeneratorExecutor implements IGeneratorExecutor {
	...
	def String getPackage(Entity entity) {
		service.configuration.groupId + '.' + service.domain + '.' + service.name + '.entity.' + entity.name
	}
	...
	def generateEntity(Entity entity) {
		val basePakage = 'mod-gen/server/src/main/java/'
